...
class EscapeCodes(out: OutputStream) {
  ...
  /*   CPR   */
  def cursorPosition(): (Int, Int) = { val r = getReport(() => CSI(6, 'n'), 2, 'R'); (r(1), r(0)) }
  ...
  /* dtterm */
  def screenSize(): (Int, Int) = { val r = getReport(() => CSI(18, 't'), 3, 't'); (r(2), r(1)) }
  ...
  /* dtterm */
  def windowPosition(): (Int, Int) = { val r = getReport(() => CSI(13, 't'), 3, 't'); (r(2), r(1)) }
  /* dtterm */
  def windowSize(): (Int, Int) = { val r = getReport(() => CSI(14, 't'), 3, 't'); (r(2), r(1)) }
